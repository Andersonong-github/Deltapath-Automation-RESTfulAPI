...
def run_context_task(log_func, shared_data):
    ...
    custom_suffixes = shared_data.get("context_suffixes")
    if custom_suffixes:
        suffixes = custom_suffixes
        log_func(f"📋 Using custom context suffixes: {suffixes}")
    ...
    expected = len(custom_list) if custom_list else len(suffixes)
    log_func(f"✅ Task 2 complete: {success_count}/{expected} contexts created")
    return success_count == expected
