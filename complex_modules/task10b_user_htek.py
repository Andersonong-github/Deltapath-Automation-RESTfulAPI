...
def run_user_htek_task(log_func, shared_data):
    ...
    exts = parse_ext_range(ext_range, strip_six=False)
    ...
    for i, ext in enumerate(exts):
        ...
        if usernames and i < len(usernames) and usernames[i].strip():
            from utils.name_split import split_name
            first_name, last_name = split_name(usernames[i])
        elif i == 0:
            first_name, last_name = f"User{last4}", "Manager"
        else:
            first_name, last_name = f"User{last4}", f"User {i}"

        if emails and i < len(emails) and emails[i].strip():
            user_email = emails[i].strip()
        else:
            user_email = ""
        ...
        log_func(f"Creating Htek User: {ext} ({last_name})")
        try:
