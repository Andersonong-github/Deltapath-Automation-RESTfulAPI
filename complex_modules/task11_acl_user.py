...
def run_acl_user_task(log_func, shared_data):
    ...
    # ========== GET #2: ACL group list → find Managers/Supervisors/Users IDs + customer_id ==========
    managers_acl_id = None
    supervisors_acl_id = None
    users_acl_id = None
        ...
        if resp.status_code == 200:
            ...
            for row in rows:
                ...
                if name.lower() == f"{group_name}_managers".lower():
                    managers_acl_id = gid
                elif name.lower() == f"{group_name}_supervisors".lower():
                    supervisors_acl_id = gid
    ...
    acl_managers = [e for e in parse_ext_range(shared_data.get("acl_managers", ""), strip_six=False)]
    acl_supervisors = [e for e in parse_ext_range(shared_data.get("acl_supervisors", ""), strip_six=False)]

    if not users_acl_id:
        log_func(f"Could not find ACL group '{group_name}_Users'. Run Task 8 first.")
        return False
    if acl_managers and not managers_acl_id:
        log_func(f"Could not find ACL group '{group_name}_Managers'. Run Task 8 first.")
        return False
    if acl_supervisors and not supervisors_acl_id:
        log_func(f"Could not find ACL group '{group_name}_Supervisors'. Run Task 8 first.")
        return False
    log_func(f"ACL groups: _Managers ID={managers_acl_id}, _Supervisors ID={supervisors_acl_id}, "
             f"_Users ID={users_acl_id}, customer_id={customer_id}, managers={acl_managers}, "
             f"supervisors={acl_supervisors}")
    ...
    for gi, exts in enumerate(groups, 1):
        ...
        for i, ext in enumerate(exts):
            if ext in acl_managers:
                role = "manager"
            elif ext in acl_supervisors:
                role = "supervisor"
            else:
                role = "user"
            privilege = "manager" if role in ("manager", "supervisor") else "limited"
            acl_group_id = (managers_acl_id if role == "manager"
                            else supervisors_acl_id if role == "supervisor"
                            else users_acl_id)
            ...
            log_func(f"  {role.capitalize()}: Updating ACL user {username} (id={acl_user_id}, aclgroup={acl_group_id})...")
            try:
                ...
                if update_resp.status_code == 200 and api_ok:
                    log_func(f"  ACL user '{username}' updated as {role}")
                    success_count += 1
                else:
