...
class RestClient:
    ...
    def search_users_by_ext(self, ext_prefix, customer_id=""):
        try:
            ...
            for row in rows:
                ...
                name = row.get("name") or ""
                gid = str(row.get("customerId") or row.get("groupId") or row.get("group") or "")
                if not (ext.lower().startswith(prefix_lower) or
                    ...
                    continue
                if customer_id and gid != customer_id:
                    continue
                results.append({
                    "ext": ext,
