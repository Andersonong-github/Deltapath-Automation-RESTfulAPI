...
# ========== Optional Task (Search / Delete) ==========
    ...
    def _build_global_params(self, parent, tag):
        ...
        record_row = ttk.Frame(user_frame if tag == "complex" else param_frame)
        record_row.pack(fill="x", pady=(0, 4))

        if tag == "complex":
            ext_rec = ttk.Frame(record_row)
            ext_rec.pack(side="left", fill="both", expand=True)
            ...
            usr_rec = ttk.Frame(record_row)
            usr_rec.pack(side="left", fill="both", expand=True, padx=(4, 0))
            ttk.Label(usr_rec, text="Username:", font=st.F["small_heading"]).pack(anchor="w")
            username_text = tk.Text(usr_rec, height=5, width=16, font=st.F["mono_small"],
                                    bg=st.C["record_bg"], fg=st.C["record_fg"], relief="solid", borderwidth=1)
            username_text.pack(fill="x")

            email_rec = ttk.Frame(record_row)
            email_rec.pack(side="left", fill="both", expand=True, padx=(4, 0))
            ttk.Label(email_rec, text="Email:", font=st.F["small_heading"]).pack(anchor="w")
            email_text = tk.Text(email_rec, height=5, width=20, font=st.F["mono_small"],
                                 bg=st.C["record_bg"], fg=st.C["record_fg"], relief="solid", borderwidth=1)
            if session.get("emails"):
                email_text.insert("1.0", session.get("emails"))
            ...
            htek_model_var = tk.StringVar(value=session.get("htek_model", "uc902g"))
            ...
            user_rec_row = ttk.Frame(user_frame)
            user_rec_row.pack(fill="x", pady=(0, 4))
            left_rec = ttk.Frame(user_rec_row)
            ...
            # ========== Frame 10: ACL User ==========
            acl_user_frame = ttk.LabelFrame(param_frame, text="ACL User:", padding=4)
            acl_user_frame.pack(fill="x", pady=(0, 4))
            acl_user_roles = []
            for role, attr_key, default_checked in (("Managers", "acl_managers", True),
                                                    ("Supervisors", "acl_supervisors", True),
                                                    ("Users", "acl_users", False)):
                role_row = ttk.Frame(acl_user_frame)
                role_row.pack(fill="x", pady=(0, 2))
                role_var = tk.BooleanVar(value=default_checked)
                role_cb = ttk.Checkbutton(role_row, text=f"{role}:", variable=role_var)
                role_cb.pack(side="left", padx=(0, 4))
                role_ent = ttk.Entry(role_row, width=22)
                role_ent.insert(0, session.get(attr_key, ""))
                role_ent.pack(side="left")
                setattr(self, f"{tag}_acl_{attr_key}_ent", role_ent)
                setattr(self, f"{tag}_acl_{attr_key}_cb", role_var)
                acl_user_roles.append((role_var, role_cb, role_ent))

            acl_user_task_var = task_vars["ACL User"]

            def _set_acl_user_state(*a):
                enabled = acl_user_task_var.get()
                for role_var, role_cb, role_ent in acl_user_roles:
                    role_cb.configure(state="normal" if enabled else "disabled")
                    role_ent.configure(state="normal" if enabled and role_var.get() else "disabled")

            for role_var, role_cb, role_ent in acl_user_roles:
                role_var.trace_add("write", _set_acl_user_state)
            acl_user_task_var.trace_add("write", _set_acl_user_state)
            _set_acl_user_state()
        else:
        ...
        def _fill_ext():
            ...
            exts = self._parse_ext_range(inputs["user_ext"].get().strip(), strip_six=(tag != "complex"))
            for e in exts:
        ...
        record_btn_frame = ttk.Frame(user_frame if tag == "complex" else param_frame)
        ...
        setattr(self, f"{tag}_username_text", username_text)
        setattr(self, f"{tag}_email_text", email_text)
    ...
    def toggle_url_proto(self):
        current = self.url_proto.cget("text")
        new = "http://" if current == "https://" else "https://"
        self.url_proto.config(text=new)
        self.log(f"URL protocol switched to {new}")
    ...
    def start_thread(self, tag):
        ...
        # Validate Username column (complex only, optional)
        if tag == "complex":
            username_text = getattr(self, f"{tag}_username_text", None)
            if username_text:
                raw_un = username_text.get("1.0", "end").strip()
                if raw_un:
                    un_lines = [l.strip() for l in raw_un.split("\n") if l.strip()]
                    exts = self._parse_ext_range(inputs["user_ext"].get().strip(), strip_six=False)
                    if not exts:
                        self.log("Invalid User Extension / Extension Range.")
                        return
                    if len(un_lines) != len(exts):
                        self.log(f"Username: expected {len(exts)} line(s) for {len(exts)} extension(s), got {len(un_lines)}.")
                        return
                    setattr(self, f"{tag}_usernames", un_lines)
                    self.log(f"Username column validated for {len(un_lines)} extension(s).")
                else:
                    setattr(self, f"{tag}_usernames", [])

        # Validate Email column (complex only, optional)
        if tag == "complex":
            email_text = getattr(self, f"{tag}_email_text", None)
            if email_text:
                raw_em = email_text.get("1.0", "end").strip()
                if raw_em:
                    em_lines = [l.strip() for l in raw_em.split("\n") if l.strip()]
                    exts = self._parse_ext_range(inputs["user_ext"].get().strip(), strip_six=False)
                    if not exts:
                        self.log("Invalid User Extension / Extension Range.")
                        return
                    if len(em_lines) != len(exts):
                        self.log(f"Email: expected {len(exts)} line(s) for {len(exts)} extension(s), got {len(em_lines)}.")
                        return
                    setattr(self, f"{tag}_emails", em_lines)
                    self.log(f"Email column validated for {len(em_lines)} extension(s).")
                else:
                    setattr(self, f"{tag}_emails", [])
        ...
        # Validate ACL Group sub-options (complex only)
        if tag == "complex" and task_vars["ACL Group"].get():
            checks = getattr(self, f"{tag}_acl_checks", [])
            acl_selected = [s for s, chk, _ in checks if chk.get()]
            if not acl_selected:
                self.log("ACL Group selected: please check at least one of _Managers/_Supervisors/_Users.")
                return
            setattr(self, f"{tag}_acl_group_selection", acl_selected)
            self.log(f"ACL Group selection: {acl_selected}")

        # Validate User Profile sub-options (complex only)
        if tag == "complex" and task_vars["User Profile"].get():
            checks = getattr(self, f"{tag}_profile_checks", [])
            profile_selected = [f"Class_{num}" for num, _, chk, _ in checks if chk.get()]
            if not profile_selected:
                self.log("User Profile selected: please check at least one Class.")
                return
            setattr(self, f"{tag}_profile_classes_selection", profile_selected)
            self.log(f"User Profile classes: {profile_selected}")
    ...
    def execute_pipeline(self, tag):
        ...
        shared_data["usernames"] = getattr(self, f"{tag}_usernames", [])
        shared_data["emails"] = getattr(self, f"{tag}_emails", [])
        for acl_role, acl_key in (("managers", "acl_managers"),
                                  ("supervisors", "acl_supervisors"),
                                  ("users", "acl_users")):
            ent = getattr(self, f"{tag}_acl_{acl_role}_ent", None)
            cb = getattr(self, f"{tag}_acl_{acl_role}_cb", None)
            val = ent.get().strip() if ent and (cb is None or cb.get()) else ""
            shared_data[acl_key] = val
        shared_data["context_suffixes"] = getattr(self, f"{tag}_ctx_suffixes", None)
        shared_data["pg_spec"] = getattr(self, f"{tag}_pg_spec", None)
        shared_data["acl_group_selection"] = getattr(self, f"{tag}_acl_group_selection", None)
        shared_data["profile_classes_selection"] = getattr(self, f"{tag}_profile_classes_selection", None)
    ...
    def _on_opt_search_key(self, event=None):
        if event and event.keysym in ("Return", "Up", "Down", "Tab", "Escape"):
            return
        text = self.opt_search_entry.get().strip()
        if not hasattr(self, "opt_all_groups") or not self.opt_all_groups:
            return
        if not text:
            self.opt_group_combo["values"] = self.opt_all_groups
            return
        t = text.lower()
        if t.isdigit():
            filtered = [g for g in self.opt_all_groups if f"  [{t}" in g.lower()]
        else:
            filtered = [g for g in self.opt_all_groups if g.lower().startswith(t)]
        self.opt_group_combo["values"] = filtered
        if filtered:
            self.opt_group_combo.event_generate("<Down>")
        else:
            self.opt_group_combo.event_generate("<Escape>")
    ...
    def _parse_ext_range(self, ext_range, strip_six=True):
        from utils.ext_parser import parse_ext_range
        return parse_ext_range(ext_range, strip_six=strip_six)
    ...
    def _get_global_session_path(self, tag):
        fname = "standard_parameter.json" if tag == "standard" else "complex_parameter.json"
        return os.path.join(current_dir, fname)

    def _load_global_session(self, tag):
        path = self._get_global_session_path(tag)
        data = {}
        ...
        if data:
            return data
        legacy = os.path.join(current_dir,
                              "standard_global_parameters.json" if tag == "standard"
                              else "complex_global_parameters.json")
        if os.path.exists(legacy):
            try:
                with open(legacy, "r", encoding="utf-8") as f:
                    data = json.load(f)
                if data:
                    self._write_global_session(tag, data)
                    return data
            except Exception:
                pass
        legacy_main = self._load_last_session()
        fields = ["group_name", "group_code", "unified_limit", "host_ip", "port",
                  "context_prefix", "perm_group_prefix", "inbound_ranges", "user_ext", "htek_model"]
        migrated = {}
        for k in fields:
            if k in legacy_main:
                migrated[k] = legacy_main[k]
        if migrated:
            self._write_global_session(tag, migrated)
            return migrated
        return {}

    def _write_global_session(self, tag, data):
        try:
            with open(self._get_global_session_path(tag), "w", encoding="utf-8") as f:
    ...
    def _gather_global_params(self, tag):
        inputs = getattr(self, f"{tag}_inputs", {})
        spin = getattr(self, f"{tag}_unified_spin", None)
        htek_model = getattr(self, f"{tag}_htek_model_var", None)
        data = {
            "group_name": inputs.get("group_name", tk.Entry).get().strip() if inputs else "",
            "group_code": inputs.get("group_code", tk.Entry).get().strip() if inputs else "",
            "unified_limit": spin.get() if spin else "10",
            "host_ip": inputs.get("host_ip", tk.Entry).get().strip() if inputs else "",
            "port": inputs.get("port", tk.Entry).get().strip() if inputs else "",
            "context_prefix": inputs.get("context_prefix", tk.Entry).get().strip() if inputs else "",
            "perm_group_prefix": inputs.get("perm_group_prefix", tk.Entry).get().strip() if inputs else "",
            "inbound_ranges": inputs.get("inbound_ranges", tk.Entry).get().strip() if inputs else "",
            "user_ext": inputs.get("user_ext", tk.Entry).get().strip() if inputs else "",
            "htek_model": htek_model.get() if htek_model else "uc902g",
        }
        username_text = getattr(self, f"{tag}_username_text", None)
        if username_text:
            data["usernames"] = username_text.get("1.0", "end").strip()
        email_text = getattr(self, f"{tag}_email_text", None)
        if email_text:
            data["emails"] = email_text.get("1.0", "end").strip()
        for acl_role, acl_key in (("managers", "acl_managers"),
                                  ("supervisors", "acl_supervisors"),
                                  ("users", "acl_users")):
            ent = getattr(self, f"{tag}_acl_{acl_role}_ent", None)
            if ent:
                data[acl_key] = ent.get().strip()
        return data
    ...
    def _save_last_session(self):
        ...
        std_data = self._gather_global_params("standard")
        try:
            with open(self._get_global_session_path("standard"), "w", encoding="utf-8") as f:
                json.dump(std_data, f, ensure_ascii=False, indent=2)
        except Exception:
            pass

        cplx_data = self._gather_global_params("complex")
        try:
            with open(self._get_global_session_path("complex"), "w", encoding="utf-8") as f:
                json.dump(cplx_data, f, ensure_ascii=False, indent=2)
        except Exception:
            pass

    def on_close(self):
        try:
            siptrunk = getattr(self, "opt_siptrunkstatus", None)
            if siptrunk and siptrunk.auto_refresh_job:
                self.root.after_cancel(siptrunk.auto_refresh_job)
                siptrunk.auto_refresh_job = None
        except Exception:
            pass
