...
- **SIP Trunk Status page** — Live table of active SIP trunk peers (host/port/status) with peer-ID filter
...
A **fourth notebook tab** that shows active SIP trunk peers on a fixed host (`202.179.100.99`) in an Excel-style table (Name / Host / Port / Status).

- Fetches automatically after login
- **↻ Refresh** button re-fetches the list
- **Peer ID dropdown** filters the table by a chosen trunk (also refreshed via its own ↻ icon)
