...
class OptCallForward:
    ...
    def _create_ui(self):
        ...
        self._lbl = ttk.Label(row, text="Call Forward", anchor="w")
    ...
    def _fetch_callforwards(self):
        resp = self.rc.get("RESTful/index.php/v1/get/numberingplan/numbercallforward/view/list")
        if resp.status_code != 200:
            raise Exception(f"Call Forward list API HTTP {resp.status_code}")
        data = resp.json()
        return data.get("data") or data.get("list") or data.get("rows") or []

    def _fetch_number_group(self, number):
        try:
            resp = self.rc.get("RESTful/index.php/numberingplan/number/view/list",
                               {"type": "all", "query": number, "page": 1,
                                "start": 0, "limit": 100})
            if resp.status_code != 200:
                return ""
            data = resp.json()
            ...
            return rows[0].get("group", "") if rows else ""
        except Exception:
            return ""
    ...
    def search(self, keyword, customer_id=""):
        name = keyword.strip()
        if not name:
            self.log("Search Call Forward: please enter a keyword.")
            self.app.root.after(0, lambda: self._draw_indicator(3))
            return (False, [])
        self.app.root.after(0, lambda: self._draw_indicator(1))
        self.log(f"Searching for call forward entries with group: '{name}'...")
        try:
            if not self.rc:
            ...
            rows = self._fetch_callforwards()
            self.log(f"Fetched {len(rows)} call forward entry/entries, resolving group per number...")
            results = []
            ...
            for row in rows:
                ...
                call_forward = str(row.get("call_forward", "") or "")
                if not number:
                    continue
                grp = self._fetch_number_group(number)
                gl = grp.lower()
                if cid:
                    matched = (gl == prefix_lower or gl == code_lower or grp == cid)
                    if not matched:
                        continue
                elif code_lower:
                    ...
                    if not (gl.startswith(prefix_lower) or number.startswith(name)):
                        continue
                results.append({
                    "number": number,
            ...
            if results:
                ...
                _names = ", ".join(f"{r.get('number', '')}->{r.get('call_forward', '')}" for r in results)
                ...
                self.log(f"Found {len(results)} call forward entries:")
                for r in results:
                    self.log(f"   number: {r.get('number','')} | call forward: {r.get('call_forward','')}")
                return (True, results)
            else:
                self.found_items = []
                ...
                self.app.root.after(0, lambda: self._draw_indicator(3))
                self.log(f"No call forward entries found for '{name}'")
                return (False, [])
    ...
    def delete(self):
        ...
        if not items:
            self.log("No call forward entries to delete. Search first.")
            return 0
        success_count = 0
        for item in items:
            number = item.get("number", "")
            call_forward = item.get("call_forward", "")
            if not number:
                continue
            self.log(f"Deleting call forward for number '{number}' (-> {call_forward})...")
            try:
                resp = self.rc.delete(f"RESTful/index.php/v1/delete/numberingplan/numbercallforward/{number}")
                try:
                    d = resp.json()
                ...
                if resp.status_code == 200 and api_ok:
                    self.log(f"Deleted call forward: {number} - {msg}")
                    success_count += 1
                else:
        ...
        if success_count == len(items):
            self.log(f"All {success_count} call forward entries deleted")
            self.app.root.after(0, lambda: (
                self.result_label.configure(state="normal"),
                self.result_label.delete(0, "end"),
                self.result_label.configure(state="readonly")
