...
class OptUser:
    ...
    def _device_desc(self, r):
        deter = str(r.get("deter", "")).strip()
        if deter == "0":
            return "Softphone"
        if deter == "1":
            return f"MAC {r.get('type','')} {r.get('model','')}".strip()
        if deter == "2":
            return f"Own CPE {r.get('cpe_model','')}".strip()
        if deter == "3":
            return "Analogue Line"
        if deter == "4":
            return "Polycom Room System"
        return f"deter={deter}"
    ...
    def search(self, keyword, customer_id=""):
        ...
        try:
            customer_code = ""
            if self.rc and not name.isdigit():
                customer_code = self.rc.get_customer_code_by_name(name)
            self.log(f"Resolved: group='{name}', customer_id='{customer_id}', code='{customer_code}'")
            results = self.rc.search_users_by_ext(name, customer_id=customer_id, code=customer_code) if self.rc else None
            if results and len(results) > 0:
                if self.rc:
                    self.log(f"Fetching detail for {len(results)} user(s)...")
                    for r in results:
                        detail = self.rc.get_user_detail(r.get("id", "") or r.get("ext", ""))
                        if detail:
                            r["deter"] = detail.get("deter", r.get("deter", ""))
                            r["type"] = detail.get("type", r.get("type", ""))
                            r["model"] = detail.get("model", r.get("model", ""))
                            r["cpe_model"] = detail.get("cpe_model", r.get("cpe_model", ""))
                self.found_items = results
                _names = ", ".join(f"{r.get('ext', '')} [{self._device_desc(r)}]" for r in results)
                ...
                for r in results:
                    self.log(f"   ext: {r.get('ext','')} | name: {r.get('name','')} | group: {r.get('group','')} | device: {self._device_desc(r)}")
                return (True, results)
            else:
                self.found_items = []
                if customer_id and self.rc:
                    self.log(f"Debug user filter: customer_id='{customer_id}', code='{customer_code}'")
