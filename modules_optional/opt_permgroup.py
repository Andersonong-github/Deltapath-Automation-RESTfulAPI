...
class OptPermGroup:
    ...
    def _create_ui(self):
        ...
        self._lbl = ttk.Label(row, text="Perm Group", width=10, anchor="w")
    ...
    def _fetch_permgroup_details_by_customer(self, customer_id, group_name, existing_ids):
        fetched = []
        list_results = self.rc.search_permgroups_by_prefix("") if self.rc else []
        for r in (list_results or []):
            title = r.get("contextTitle", "") or r.get("title", "") or ""
            if not title or title in existing_ids:
                continue
            gn = str(r.get("groupName", "") or "")
            if gn != group_name:
                continue
            detail = self.rc.get_permgroup_detail(title) if self.rc else None
    ...
    def search(self, keyword, customer_id=""):
        ...
        self.log(f"Searching for permission groups with keyword: '{q}'...")
        try:
            results = []
            resolved_name = ""
            ...
            results = self._fetch_permgroup_details_by_customer(resolved_id, resolved_name, set())
            ...
            if results:
                ...
                self.app.root.after(0, lambda: self._draw_indicator(3))
                self.log(f"No permission groups found for '{q}'")
                return (False, [])
