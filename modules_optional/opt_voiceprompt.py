...
class OptVoicePrompt:
    ...
    def _create_ui(self):
        ...
        self._lbl = ttk.Label(row, text="Voice Prompt", anchor="w")
    ...
    def _fetch_rows(self):
        resp = self.rc.get("RESTful/index.php/configuration/voiceprompt/view/list",
    ...
    def search(self, keyword, customer_id=""):
        name = keyword.strip()
        if not name:
            self.log("Search Voice Prompt: please enter a keyword.")
            self.app.root.after(0, lambda: self._draw_indicator(3))
            return (False, [])
        self.app.root.after(0, lambda: self._draw_indicator(1))
        self.log(f"Searching for voice prompts with group: '{name}'...")
        try:
            if not self.rc:
            ...
            for row in rows:
                prompt_group = str(row.get("prompt_group", "") or "")
                prompt_name = row.get("prompt_name", "") or ""
                prompt_id = row.get("prompt_id", "") or ""
                gl = prompt_group.lower()
                if cid:
                    ...
                    if not (gl.startswith(prefix_lower)
                            or prompt_name.lower().startswith(prefix_lower)):
                        continue
                results.append({
                    "id": prompt_id,
            ...
            if results:
                ...
                _names = ", ".join(r.get('prompt_name', '') for r in results)
                ...
                self.log(f"Found {len(results)} voice prompts:")
                for r in results:
                    self.log(f"   {r.get('prompt_name','')} (id: {r.get('id','')})")
                return (True, results)
            else:
                self.found_items = []
                ...
                self.app.root.after(0, lambda: self._draw_indicator(3))
                self.log(f"No voice prompts found for '{name}'")
                return (False, [])
    ...
    def delete(self):
        ...
        if not items:
            self.log("No voice prompts to delete. Search first.")
            return 0
        success_count = 0
        for item in items:
            pid = item.get("id", "")
            ...
            if not pid:
                continue
            self.log(f"Deleting voice prompt '{name}' (id: {pid})...")
            try:
                resp = self.rc.delete(f"RESTful/index.php/v1/configuration/voiceprompt/{pid}")
                try:
                    d = resp.json()
                ...
                if resp.status_code == 200 and api_ok:
                    self.log(f"Deleted voice prompt: {name} - {msg}")
                    success_count += 1
                else:
        ...
        if success_count == len(items):
            self.log(f"All {success_count} voice prompts deleted")
            self.app.root.after(0, lambda: (
                self.result_label.configure(state="normal"),
                self.result_label.delete(0, "end"),
                self.result_label.configure(state="readonly")
