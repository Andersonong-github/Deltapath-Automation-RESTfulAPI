...
class OptAgents:
    ...
    def _create_ui(self):
        ...
        self._lbl = ttk.Label(row, text="Agents", anchor="w")
    ...
    def _fetch_rows(self):
        resp = self.rc.get("RESTful/index.php/get/callcentre/agent/view/list",
                           {"display_mode": "list", "page": 1, "start": 0, "limit": 1000})
        if resp.status_code != 200:
            raise Exception(f"API HTTP {resp.status_code}")
        data = resp.json()
        return data.get("list") or data.get("rows") or data.get("data") or []

    def _fetch_acl_privileges(self, agentid):
        try:
            resp = self.rc.get("RESTful/index.php/v1/get/configuration/acluser/view/list",
                               {"query": agentid, "page": 1, "start": 0, "limit": 100})
            if resp.status_code != 200:
                return ""
            data = resp.json()
            ...
            for row in rows:
                if str(row.get("username", "")) == str(agentid):
                    return str(row.get("privileges", "") or "")
            return str(rows[0].get("privileges", "") or "") if rows else ""
        except Exception:
            return ""
    ...
    def search(self, keyword, customer_id=""):
        ...
        try:
            ...
            for row in rows:
                gname = str(row.get("generalgroupid", "") or "")
                ...
                if cid:
                    ...
                    if not (gl.startswith(prefix_lower)
                            or agentid.lower().startswith(prefix_lower)
                            or firstname.lower().startswith(prefix_lower)
                            or lastname.lower().startswith(prefix_lower)):
                        continue
                self.log(f"Fetching ACL privileges for agent '{agentid}'...")
                privileges = self._fetch_acl_privileges(agentid)
                results.append({
            ...
            if results:
                ...
                _names = ", ".join(
                    f"{r.get('agentid','')} {r.get('firstname','')} {r.get('lastname','')} (priv: {r.get('privileges','')})".strip()
                    for r in results)
                ...
                self.log(f"Found {len(results)} agents:")
                for r in results:
                    self.log(f"   agentid: {r.get('agentid','')} | firstname: {r.get('firstname','')} | lastname: {r.get('lastname','')} | privileges: {r.get('privileges','')}")
                return (True, results)
            else:
                self.found_items = []
                ...
                self.app.root.after(0, lambda: self._draw_indicator(3))
                self.log(f"No agents found for '{name}'")
                return (False, [])
    ...
    def delete(self):
        ...
        if not items:
            self.log("No agents to delete. Search first.")
            return 0
        success_count = 0
        for item in items:
            aid = item.get("id", "")
            ...
            if not aid:
                continue
            self.log(f"Deleting agent '{agentid}' (id: {aid})...")
            try:
                resp = self.rc.delete(f"RESTful/index.php/v1/callcentre/agent/{aid}")
                try:
                    d = resp.json()
                ...
                if resp.status_code == 200 and api_ok:
                    self.log(f"Deleted agent: {agentid} - {msg}")
                    success_count += 1
                else:
                    self.log(f"Failed ({resp.status_code}): {msg}")
            except Exception as e:
                self.log(f"Error deleting {agentid}: {e}")
        if success_count == len(items):
            self.log(f"All {success_count} agents deleted")
            self.app.root.after(0, lambda: (
                self.result_label.configure(state="normal"),
                self.result_label.delete(0, "end"),
                self.result_label.configure(state="readonly")
