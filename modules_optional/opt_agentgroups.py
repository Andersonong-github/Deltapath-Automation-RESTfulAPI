...
class OptAgentGroups:
    ...
    def _create_ui(self):
        ...
        self._lbl = ttk.Label(row, text="Agent Groups", anchor="w")
    ...
    def _fetch_rows(self):
        resp = self.rc.get("RESTful/index.php/get/callcentre/agentgroup/view/list",
    ...
    def search(self, keyword, customer_id=""):
        name = keyword.strip()
        if not name:
            self.log("Search Agent Groups: please enter a keyword.")
            self.app.root.after(0, lambda: self._draw_indicator(3))
            return (False, [])
        self.app.root.after(0, lambda: self._draw_indicator(1))
        self.log(f"Searching for agent groups with group: '{name}'...")
        try:
            if not self.rc:
            ...
            for row in rows:
                gname = str(row.get("generalgroup", "") or "")
                grp_name = row.get("name", "") or ""
                grp_id = row.get("id", "") or ""
                gl = gname.lower()
                if cid:
                    ...
                    if not (gl.startswith(prefix_lower)
                            or grp_name.lower().startswith(prefix_lower)):
                        continue
                results.append({
                    "id": grp_id,
                    "name": grp_name,
            ...
            if results:
                ...
                self.log(f"Found {len(results)} agent groups:")
                for r in results:
                ...
                self.app.root.after(0, lambda: self._draw_indicator(3))
                self.log(f"No agent groups found for '{name}'")
                return (False, [])
    ...
    def delete(self):
        ...
        if not items:
            self.log("No agent groups to delete. Search first.")
            return 0
        success_count = 0
        for item in items:
            gid = item.get("id", "")
            ...
            if not gid:
                continue
            self.log(f"Deleting agent group '{name}' (id: {gid})...")
            try:
                resp = self.rc.delete(f"RESTful/index.php/v1/callcentre/agentgroup/{gid}")
                try:
                    d = resp.json()
                ...
                if resp.status_code == 200 and api_ok:
                    self.log(f"Deleted agent group: {name} - {msg}")
                    success_count += 1
                else:
        ...
        if success_count == len(items):
            self.log(f"All {success_count} agent groups deleted")
            self.app.root.after(0, lambda: (
                self.result_label.configure(state="normal"),
                self.result_label.delete(0, "end"),
                self.result_label.configure(state="readonly")
