...
class OptContext:
    ...
    def _create_ui(self):
        ...
        ttk.Label(row, text="Context", width=10, anchor="w").pack(side="left")
    ...
    def search(self, keyword):
        ...
        self.log(f"Searching for contexts with prefix: '{prefix}'...")
        try:
            results = self.rc.search_contexts_by_prefix(prefix) if self.rc else None
            if results and len(results) > 0:
                ...
                self.log(f"No contexts found: {msg}")
                return (False, [])
