...
class OptCallPickup:
    ...
    def _create_ui(self):
        ...
        self._lbl = ttk.Label(row, text="Call Pickup", width=10, anchor="w")
    ...
    def _fetch_rows(self):
        resp = self.rc.get("RESTful/index.php/v1/get/user/callpickupgroup/view/list",
    ...
    def search(self, keyword, customer_id=""):
        name = keyword.strip()
        if not name:
            self.log("Search Call Pickup: please enter a keyword.")
            self.app.root.after(0, lambda: self._draw_indicator(3))
            return (False, [])
        self.app.root.after(0, lambda: self._draw_indicator(1))
        self.log(f"Searching for call pickup groups with group: '{name}'...")
        try:
            if not self.rc:
            ...
            for row in rows:
                ...
                group_id = row.get("groupID", "") or ""
                gname = str(row.get("groupTitle") or row.get("groupname") or "")
                gl = generalgroup.lower()
                if cid:
                    matched = (gl == prefix_lower
                               or gl == code_lower
                               or str(self.rc._extract_group(row)).lower() == cid)
                    if not matched:
                        continue
                elif code_lower:
                    ...
                    if not (gl.startswith(prefix_lower)
                            or gname.lower().startswith(prefix_lower)):
                        continue
                results.append({
                    "groupID": group_id,
            ...
            if results:
                ...
                _names = ", ".join(r.get('groupTitle', '') for r in results)
                ...
                self.log(f"Found {len(results)} call pickup groups:")
                for r in results:
                    self.log(f"   {r.get('groupTitle','')} (groupID: {r.get('groupID','')})")
                return (True, results)
            else:
                self.found_items = []
                ...
                self.app.root.after(0, lambda: self._draw_indicator(3))
                self.log(f"No call pickup groups found for '{name}'")
                return (False, [])
    ...
    def delete(self):
        ...
        if not items:
            self.log("No call pickup groups to delete. Search first.")
            return 0
        success_count = 0
        for item in items:
            ...
            if not gid:
                continue
            self.log(f"Deleting call pickup group '{title}' (groupID: {gid})...")
            try:
                resp = self.rc.post(f"RESTful/index.php/v1/delete/user/callpickupgroup/{gid}")
                try:
                    d = resp.json()
                ...
                if resp.status_code == 200 and api_ok:
                    self.log(f"Deleted call pickup group: {title} - {msg}")
                    success_count += 1
                else:
        ...
        if success_count == len(items):
            self.log(f"All {success_count} call pickup groups deleted")
            self.app.root.after(0, lambda: (
                self.result_label.configure(state="normal"),
                self.result_label.delete(0, "end"),
                self.result_label.configure(state="readonly")
