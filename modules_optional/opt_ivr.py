...
class OptIVR:
    ...
    def _create_ui(self):
        ...
        self._lbl = ttk.Label(row, text="IVR", anchor="w")
    ...
    def _first_char(self, text):
        t = text.strip()
        if not t:
            return ""
        return t[0] if (t[0].isalpha() or t[0].isdigit()) else ""

    def _fetch_rows(self, query_char):
        resp = self.rc.get("RESTful/index.php/ivr/route/view/list",
                           {"query": query_char, "page": 1, "start": 0,
                            "limit": 100, "node": "root"})
        if resp.status_code != 200:
            raise Exception(f"API HTTP {resp.status_code}")
        data = resp.json()
        return data.get("children") or data.get("list") or data.get("rows") or data.get("data") or []

    def search(self, keyword, customer_id=""):
        name = keyword.strip()
        if not name:
            self.log("Search IVR: please enter a keyword.")
            self.app.root.after(0, lambda: self._draw_indicator(3))
            return (False, [])
        self.app.root.after(0, lambda: self._draw_indicator(1))
        self.log(f"Searching for IVR routes with group: '{name}'...")
        try:
            if not self.rc:
            ...
            query_char = self._first_char(resolved_name) or self._first_char(customer_code)
            self.log(f"IVR query char: '{query_char}'")
            rows = self._fetch_rows(query_char)
            results = []
            ...
            for row in rows:
                ...
                ivr_name = row.get("ivr", "") or ""
                gl = group_field.lower()
                if cid:
                    matched = (gl == prefix_lower
                               or gl == code_lower
                               or group_field == cid)
                    if not matched:
                        continue
                    ...
                    if not (gl.startswith(prefix_lower)
                            or ivr_name.lower().startswith(prefix_lower)):
                        continue
                results.append({
                    "id": ivr_name,
                    "name": ivr_name,
            ...
            if results:
                ...
                _names = ", ".join(r.get('name', '') for r in results)
                ...
                self.log(f"Found {len(results)} IVR routes:")
                for r in results:
                    self.log(f"   {r.get('name','')}")
                return (True, results)
            else:
                self.found_items = []
                ...
                self.app.root.after(0, lambda: self._draw_indicator(3))
                self.log(f"No IVR routes found for '{name}'")
                return (False, [])
    ...
    def delete(self):
        ...
        if not items:
            self.log("No IVR routes to delete. Search first.")
            return 0
        success_count = 0
        for item in items:
            ivr_name = item.get("id", "")
            name = item.get("name", "")
            if not ivr_name:
                continue
            self.log(f"Deleting IVR '{name}'...")
            try:
                resp = self.rc.delete(f"RESTful/index.php/v1/ivr/route/menu/{ivr_name}")
                try:
                    d = resp.json()
                ...
                if resp.status_code == 200 and api_ok:
                    self.log(f"Deleted IVR: {name} - {msg}")
                    success_count += 1
                else:
        ...
        if success_count == len(items):
            self.log(f"All {success_count} IVR routes deleted")
            self.app.root.after(0, lambda: (
                self.result_label.configure(state="normal"),
                self.result_label.delete(0, "end"),
                self.result_label.configure(state="readonly")
