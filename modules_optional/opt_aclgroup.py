...
class OptAclGroup:
    ...
    def _create_ui(self):
        ...
        self._lbl = ttk.Label(row, text="ACL Group", width=10, anchor="w")
    ...
    def search(self, keyword, customer_id=""):
        ...
        try:
            ...
            results = self.rc.search_acl_groups_by_name(resolved_name, resolved_id, code=customer_code) if self.rc else None
            if results and len(results) > 0:
                ...
                self.log(f"Found {len(results)} ACL groups:")
                for r in results:
                    self.log(f"   {r.get('name','')} (id: {r.get('group_id','')})")
                return (True, results)
            else:
                self.found_items = []
                ...
                self.log(f"No ACL groups found: {msg}")
                return (False, [])
    ...
    def delete(self):
        ...
        if not items:
            self.log("No ACL groups to delete. Search first.")
            return 0
        success_count = 0
        for item in items:
            gid = item.get("group_id", "") or item.get("id", "") or item.get("ID", "")
            name = item.get("name", "")
            if not gid:
                continue
            self.log(f"Deleting ACL group '{name}' (id: {gid})...")
            try:
                resp = self.rc.post(f"RESTful/index.php/delete/configuration/aclgroup/{gid}")
                try:
                    d = resp.json()
        ...
        if success_count == len(items):
            self.log(f"All {success_count} ACL groups deleted")
            self.app.root.after(0, lambda: (
                    self.result_label.configure(state="normal"),
                    self.result_label.delete(0, "end"),
                    self.result_label.configure(state="readonly")
